...
export const handler = async (event) => {
  ...
  if (stripeEvent.type === "checkout.session.completed") {
    ...
    const amountPaidThisSession = (session.amount_total || 0) / 100; // Stripe amounts are in cents

    if (userId) {
      try {
        // Fetch the amount already recorded for this client so a repeat
        // Stripe payment (e.g. someone paying the remainder after adding
        // entries) adds to their total instead of overwriting it.
        let existingAmount = 0;
        const existingRes = await fetch(`${SUPABASE_URL}/rest/v1/payments?user_id=eq.${userId}&select=amount_paid`, {
          headers: {
        ...
        if (existingRes.ok) {
          ...
          existingAmount = Number(rows?.[0]?.amount_paid) || 0;
        }
        ...
        const res = await fetch(`${SUPABASE_URL}/rest/v1/payments`, {
          ...
          body: JSON.stringify({
            ...
            amount_paid: existingAmount + amountPaidThisSession,
          }),
        });
        if (!res.ok) {
